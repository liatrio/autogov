...
// Signer is a self-contained demonstration Sigstore CA: a local root with a
...
// exports the matching trusted root JSON. Demonstration/test infrastructure
...
// NewSigner builds the demonstration CA hierarchy for one signing identity
// (an email SAN) and OIDC issuer.
func NewSigner(identity, issuer string) (*Signer, error) {
	s := &Signer{
		identity:      identity,
		issuer:        issuer,
	...
	rootTemplate := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "autogov-agentgov-demo-root", Organization: []string{"autogov-agent-governance-demo"}},
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create demo root CA: %w", err)
	}
...
func (s *Signer) createFulcioIntermediate(rootKey *ecdsa.PrivateKey) error {
	...
	template := &x509.Certificate{
		SerialNumber:          newSerial(),
		Subject:               pkix.Name{CommonName: "autogov-agentgov-demo-fulcio", Organization: []string{"autogov-agent-governance-demo"}},
		NotBefore:             s.validityStart,
		NotAfter:              s.validityEnd,
		KeyUsage:              x509.KeyUsageCertSign | x509.KeyUsageCRLSign,
	...
	if err != nil {
		return fmt.Errorf("failed to create demo Fulcio intermediate: %w", err)
	}
	s.fulcioIntermediateKey = key
	return nil
}

func (s *Signer) createTSA(rootKey *ecdsa.PrivateKey) error {
	...
	intermediateTemplate := &x509.Certificate{
		SerialNumber:          newSerial(),
		Subject:               pkix.Name{CommonName: "autogov-agentgov-demo-tsa", Organization: []string{"autogov-agent-governance-demo"}},
		NotBefore:             s.validityStart,
		NotAfter:              s.validityEnd,
		KeyUsage:              x509.KeyUsageCertSign | x509.KeyUsageCRLSign,
	...
	if err != nil {
		return fmt.Errorf("failed to create demo TSA intermediate: %w", err)
	}
	...
	leafTemplate := &x509.Certificate{
		SerialNumber: newSerial(),
		Subject:      pkix.Name{CommonName: "autogov-agentgov-demo-tsa-leaf"},
		NotBefore:    s.validityStart,
		NotAfter:     s.validityEnd,
	...
	if err != nil {
		return fmt.Errorf("failed to create demo TSA leaf: %w", err)
	}
...
// demo TSA. The bundle carries no transparency log entry; offline
// verification uses the observer timestamp against the exported trusted root.
func (s *Signer) SignStatement(statementJSON []byte) ([]byte, error) {
	leafKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		return nil, err
	}
...
func (s *Signer) timestampResponse(sig []byte) ([]byte, error) {
...
// bundles: the demo Fulcio CA chain and the demo timestamping authority.
func (s *Signer) TrustedRootJSON() ([]byte, error) {
	fulcioCA := &root.FulcioCertificateAuthority{
		...
		URI:                 "https://demo.autogov.local/fulcio",
	}
	tsa := &root.SigstoreTimestampingAuthority{
		...
		ValidityPeriodStart: s.validityStart,
		ValidityPeriodEnd:   s.validityEnd,
		URI:                 "https://demo.autogov.local/tsa",
	}
...
func newSerial() *big.Int {
	...
	if err != nil {
		// crypto/rand failure is unrecoverable for a demo signer
		panic(err)
	}
	return serial
}
