...
// validates VSA verification result against policy requirements
// this function can be called independently for policy enforcement
func ValidateVSAVerificationResult(vsa *VSA, requirePassed bool) error {
	if vsa == nil {
		return NewValidationError("vsa", "VSA cannot be nil", nil)
	}

	result := vsa.Predicate.VerificationResult
	if result == "" {
		return NewValidationError("verificationResult", "verification result is required", nil)
	}

	// ensure result is valid
	if result != "PASSED" && result != "FAILED" && result != "UNKNOWN" {
		return NewValidationError("verificationResult", 
			fmt.Sprintf("invalid verification result: %s (must be PASSED, FAILED, or UNKNOWN)", result), nil)
	}

	// policy enforcement: if requirePassed is true, only PASSED results are acceptable
	if requirePassed && result != "PASSED" {
		return NewValidationError("verificationResult", 
			fmt.Sprintf("verification failed with result: %s (policy requires PASSED)", result), nil)
	}

	return nil
}

// validates and loads a VSA from bytes with optional policy enforcement
func ValidateAndLoadVSA(vsaBytes []byte, requirePassed bool) (*VSA, error) {
	...
	if err := ValidateVSAVerificationResult(vsa, requirePassed); err != nil {
		return nil, err
	}
